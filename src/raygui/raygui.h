...
RAYGUIAPI int GuiListViewEx(rlRectangle bounds, char **text, int count, int *scrollIndex, int *active, int *focus); // List View control, using text entries list and returning focus entry
RAYGUIAPI int GuiTabBar(rlRectangle bounds, const char *text, int *hscroll, int *active);                // Tab Bar control
RAYGUIAPI int GuiTabBarEx(rlRectangle bounds, char **text, int count, int *hscroll, int *active, int *focus); // Tab Bar control, using text entries list and returning focus entry
RAYGUIAPI int GuiMessageBox(rlRectangle bounds, const char *title, const char *message, const char *btnText, int *btnActive); // Message Box control, displays a message
RAYGUIAPI int GuiTextInputBox(rlRectangle bounds, const char *title, const char *message, char *text, int textSize, const char *btnText, int *btnActive, bool *secretViewActive); // Text Input Box control, ask for text, supports secret
RAYGUIAPI int GuiColorPicker(rlRectangle bounds, const char *text, Color *color);                        // Color Picker control, includes Color bar controls
...
RAYGUIAPI int GuiColorPickerHSV(rlRectangle bounds, const char *text, Vector3 *colorHsv);                // Color Picker control, using Hue-Saturation-Value color data, includes Color bar controls
RAYGUIAPI int GuiColorPanelHSV(rlRectangle bounds, const char *text, Vector3 *colorHsv);                 // Color Panel control, using Hue-Saturation-Value color data
//----------------------------------------------------------------------------------------------------------
...
static rlRectangle guiControlExclusiveRec = { 0 }; // Gui control exclusive bounds rectangle, used as an unique identifier
...
static int GuiFontIconBaking(Image *imFont, Font font, rlRectangle *whiteRec); // Update font image atlas to append raygui icons
...
int GuiWindowBox(rlRectangle bounds, const char *title)
{
    ...
    rlRectangle windowPanel = { bounds.x, bounds.y + (float)statusBarHeight - (float)statusBorderWidth, bounds.width, bounds.height - (float)statusBarHeight + (float)statusBorderWidth };
    rlRectangle closeButtonRec = { statusBar.x + statusBar.width - (float)statusBorderWidth - RAYGUI_WINDOWBOX_CLOSEBUTTON_HEIGHT - vPadding,
...
int GuiGroupBox(rlRectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? (int)BORDER_COLOR_DISABLED : (int)LINE_COLOR)));
...
int GuiDropdownBox(rlRectangle bounds, const char *text, int *active, bool editMode)
{
    ...
    if (!GuiGetStyle(DROPDOWNBOX, DROPDOWN_ARROW_HIDDEN))
    {
        ...
        GuiDrawText(direction? GuiIconText(ICON_ARROW_UP_FILL, NULL) : GuiIconText(ICON_ARROW_DOWN_FILL, NULL),
            RAYGUI_CLITERAL(rlRectangle){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 6, 10, 10 },
...
int GuiTextBox(rlRectangle bounds, char *text, int textSize, bool editMode)
{
...
bool GuiTextBoxMulti(rlRectangle bounds, char *text, int textSize, bool editMode)
{
    bool pressed = false;
...
int GuiSpinner(rlRectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    rlRectangle leftButtonBound = { (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH), (float)bounds.height };
    rlRectangle rightButtonBound = { (float)bounds.x + bounds.width - GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH), (float)bounds.y,
...
int GuiValueBox(rlRectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if (editMode)
    {
        ...
        rlRectangle cursor = { bounds.x + GuiGetTextWidth(textValue)/2 + bounds.width/2 + 1,
...
int GuiValueBoxFloat(rlRectangle bounds, const char *text, char *textValue, float *value, bool editMode)
{
    #if !defined(RAYGUI_VALUEBOX_MAX_CHARS)
        #define RAYGUI_VALUEBOX_MAX_CHARS  32
    #endif
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        if (guiControlExclusiveMode) // Allows to keep dragging outside of bounds
        {
            ...
            else
            {
                ...
                guiControlExclusiveRec = RAYGUI_CLITERAL(rlRectangle){ 0, 0, 0, 0 };
            }
        }
...
int GuiProgressBar(rlRectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    ...
    else
    {
        ...
        else GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height+GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)-1 }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));

        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height+GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)-1}, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        else
        {
            // Draw borders not yet reached by value
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y, bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y + bounds.height - 1, bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x + bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height+GuiGetStyle(PROGRESSBAR, BORDER_WIDTH)-1 }, 0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
...
int GuiListViewEx(rlRectangle bounds, char **text, int count, int *scrollIndex, int *active, int *focus)
{
    int result = RESULT_NONE;
    GuiState state = guiState;
...
// Tab Bar control
int GuiTabBar(rlRectangle bounds, const char *text, int *hscroll, int *active)
{
    int result = RESULT_NONE;
    int itemCount = 0;
    char **items = NULL;
...
int GuiTabBarEx(rlRectangle bounds, char **text, int count, int *hscroll, int *active, int *focus)
{
    int result = RESULT_NONE;
    //GuiState state = guiState;

    int tabItemsWidth = GuiGetStyle(TABBAR, TAB_ITEMS_WIDTH);
    rlRectangle tabBounds = { bounds.x, bounds.y, (float)tabItemsWidth, bounds.height };
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            if (GuiGetStyle(TABBAR, TAB_CLOSE_BUTTON))
            {
                ...
                GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
                if (GuiButton(RAYGUI_CLITERAL(rlRectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, "x")) result = RESULT_TAB_CLOSE;
#else
                if (GuiButton(RAYGUI_CLITERAL(rlRectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 },
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 }, 0, BLANK, GetColor(GuiGetStyle(TABBAR, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiColorBarAlpha(rlRectangle bounds, const char *text, float *alpha)
{
    ...
    rlRectangle selector = { (float)bounds.x + (*alpha)*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2,
...
int GuiColorBarHue(rlRectangle bounds, const char *text, float *hue)
{
    ...
    if (state != STATE_DISABLED)
    {
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y, bounds.width, bounds.height/6.0f }, 
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + 1*(bounds.height/6.0f), bounds.width, bounds.height/6.0f }, 
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + 2*(bounds.height/6.0f), bounds.width, bounds.height/6.0f }, 
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + 3*(bounds.height/6.0f), bounds.width, bounds.height/6.0f }, 
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + 4*(bounds.height/6.0f), bounds.width, bounds.height/6.0f }, 
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(rlRectangle){ bounds.x, bounds.y + 5*(bounds.height/6.0f), bounds.width, bounds.height/6.0f }, 
...
int GuiMessageBox(rlRectangle bounds, const char *title, const char *message, const char *btnText, int *btnActive)
{
...
// NOTE: Button pressed is returned through btnActive parameter, 0 for window close button
int GuiTextInputBox(rlRectangle bounds, const char *title, const char *message, char *text, int textSize, const char *btnText, int *btnActive, bool *secretViewActive)
{
...
#if defined(RAYGUI_NO_ICONS)
        GuiToggle(RAYGUI_CLITERAL(rlRectangle){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y, RAYGUI_TEXTINPUTBOX_HEIGHT, RAYGUI_TEXTINPUTBOX_HEIGHT },
...
int GuiGrid(rlRectangle bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell)
{
    ...
    if (subdivs > 0)
    {
        ...
        {
            rlRectangle lineV = { bounds.x + spacing*i/subdivs, bounds.y, 1, bounds.height + 1 };
        ...
        {
            rlRectangle lineH = { bounds.x, bounds.y + spacing*i/subdivs, bounds.width + 1, 1 };
...
void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
    ...
    // ...     | 16      | rlRectangle  | Font white rectangle
    ...
    //    foreach (glyph)
    //    {
    //       ...  | 16      | rlRectangle  | Glyph rectangle (in image)
    //    }
    // }
    // else
    // {
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color)
{
    if ((guiIconFontOffsetY > 0) && (iconId < RAYGUI_ICON_MAX_FONT_BACKED))
    {
        ...
        rlRectangle srcRec = { (float)x*(RAYGUI_ICON_SIZE + 2*RAYGUI_ICON_FONT_ATLAS_PADDING) + RAYGUI_ICON_FONT_ATLAS_PADDING,
        ...
        rlRectangle dstRec = { (float)posX, (float)posY, (float)pixelSize*RAYGUI_ICON_SIZE, (float)pixelSize*RAYGUI_ICON_SIZE };
    ...
    else
    {
        ...
        for (int i = 0, y = 0; i < RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32; i++)
        {
            for (int k = 0; k < 32; k++)
            {
                ...
                {
                    GuiDrawRectangle(RAYGUI_CLITERAL(rlRectangle){ (float)posX + (k%RAYGUI_ICON_SIZE)*pixelSize,
...
static void GuiTooltip(rlRectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiControlExclusiveMode)
    {
        ...
        GuiPanel(RAYGUI_CLITERAL(rlRectangle){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, textSize.y + 8.0f*lineCount }, NULL);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(RAYGUI_CLITERAL(rlRectangle){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, textSize.y + 8.0f*lineCount }, guiTooltipPtr);
...
static int GuiFontIconBaking(Image *imFont, Font font, rlRectangle *whiteRec)
{
    int iconOffsetY = 0;
    ...
    if ((maxGlyphRecY + reqHeight) > imFont->height)
    {
        ...
        *whiteRec = RAYGUI_CLITERAL(rlRectangle){ (float)imFont->width - 2, (float)imFont->height - 2, 1, 1 };
    }
